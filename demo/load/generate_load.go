...
/*
	  Run a load test on a local fakenet

		The idea is to first launch the fakenet independently.
		At this time, it is not clear how the funded account (ftm.accounts[0])
		can be used from outside, as the fakenet doesn't publish its private key.

		Therefore, this script first creates a new private key, then uses
		the ftmclient to send funds to that key, and from then on uses that key to
		create transactions.

		It is assumed that it would be too slow to issue all transactions by attaching
		to the javascript console.

		Therefore, the idea is to then create a configurable number of transactions,
		and then fire these in a loop to the local fakenet nodes.
*/
func main() {
	if len(os.Args) != 5 {
