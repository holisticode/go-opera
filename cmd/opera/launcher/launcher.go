...
func makeP2PNode(ctx *cli.Context, cfg *config, genesisStore *genesisstore.Store) (*node.Node, *gossip.P2PTestService, func(), error) {
	p2pSrv, err := gossip.NewP2PTestService(1, 4)
	if err != nil {
...
func makeP2PTestNode(ctx *cli.Context, cfg *config, genesisStore *genesisstore.Store) *P2PTestingNode {
	...
	fmt.Println(cfg.Emitter.Validator.ID)
	fmt.Println(cfg.Emitter)
